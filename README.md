...
VoiceCalendar 是一个语音交互式智能日历工具 MVP，支持通过中文自然语言文本或麦克风录音管理日程。前端只负责录音和提交文本，添加、查看、删除等意图由后端交给 DeepSeek 解析后执行。
...
统一的自然语言日历指令入口。后端会把文本交给 DeepSeek 解析为结构化意图，再执行创建、查询或删除。
